...
ggplot(debt1, aes(x = ccarduse, y = prodebt)) + 
  geom_boxplot()
```
...
```{r}
ggplot(debt1, aes(x = ccarduse, y = incomegp)) + 
  geom_boxplot()
```
**Answer:** From the boxplot, we can see that the variability of `incomegp` is almost the same among all different levels of credit card use. But it is obvious that people with high income level are more likely to use credit card regularly.
...
```
**Answer:** The p-value of the predictor `house` is $0.015$ in this small model, which is significant. The conclusion regarding this predictor is contradictory for the two models.
...
We cannot conclude that the predictors that were dropped from the model have no relation to the response.
