...
lmod1 |>
  ...
  bold_p(t = 0.05)
```
**Answer:** The p-value of `spontaneous` is $<0.001$ so it is a significant predictor. Every one unit increase in the number of prior spontaneous abortions increases the odds of being a case by a factor of $e^{1.2} = 3.32$.\
The p-value of `induced` is $0.042<0.05$ so it is also a significant predictor. Every one unit increase in the number of prior induced abortions increases the odds of being a case by a factor of $e^{0.42} = 1.52$.
...
```{r}
lmod5 <- clogit(case ~ as.factor(spontaneous) + as.factor(induced) + 
