...
**Answer:** 
`percent_fair_or_poor_health`: Every one percent increase in the proportion of people who report fair or poor health decreases the log odds of death among confirmed cases of COVID by 0.05, controlling for all other predictors.

`percent_children_in_poverty`: Every one percent increase in the proportion of children in poverty increases the log odds of death among confirmed cases of COVID by 0.03, controlling for all other predictors.

`percent_severe_housing_problems`: Every one percent increase in the proportion of people who report severe housing problems decreases the log odds of death among confirmed cases of COVID by 0.04, controlling for all other predictors.
...
```
**Answer:** It seems that there is on potential outlier with a residual less than -5.0. There are some potential outliers with small linear predictor.
...
```{r}
county_long <- data.frame()

for (i in 1:nrow(county_data)) {
  row <- county_data[i, ]
  death_num <- row$deaths
  row <- row[rep(1, row$confirmed), ]
  row$deaths[1:death_num] <- 1
  row$deaths[(death_num + 1):nrow(row)] <- 0
  county_long <- rbind(county_long, row)
}
...
train_county <- training(data_split)
test_county <- testing(data_split)
```
...
folds <- vfold_cv(train_county, v = 5)
folds
```
...
```
**Answer:** There are more control cases with no spontaneous or induced abortions than the infertile cases. Both case and control groups have no observation with 2 or more spontaneous abortions and 2 or more induced abortions. For control group, there are more people with induced abortions than spontaneous abortions. For case group, there are more people with spontaneous abortions than induced abortions.
...
lmod1 |>
  ...
  bold_p(t = 0.05)
```
**Answer:** The p-value of `spontaneous` is $<0.001$ so it is a significant predictor. Every one unit increase in the number of prior spontaneous abortions increases the odds of being a case by a factor of $e^{1.2} = 3.32$, controlling for all other predictors.\
The p-value of `induced` is $0.042<0.05$ so it is also a significant predictor. Every one unit increase in the number of prior induced abortions increases the odds of being a case by a factor of $e^{0.42} = 1.52$, controlling for all other predictors.
...
lmod2 |>
  ...
  bold_p(t = 0.05)
```
**Answer:** The p-values of `education`, `age` and `parity` are all $\ge 0.9$ so none of them are significant predictors. This means that the odds of being a case does not change with the level of education, age or parity.
...
lmod3 |>
  ...
  bold_p(t = 0.05)
```
**Answer:** Every one unit increase in the number of prior spontaneous abortions increases the odds of being a case by a factor of $e^{2} = 7.39$, controlling for all other predictors.\
Every one unit increase in the number of prior induced abortions increases the odds of being a case by a factor of $e^{1.3} = 3.67$, controlling for all other predictors.\
Every one year increase in age increases the odds of being a case by a factor of $e^{0.04} = 1.04$, controlling for all other predictors.\
Every one unit increase in the number of parity decrease the odds of being a case by a factor of $e^{-0.83} = 0.436$, controlling for all other predictors.\
The odds of being a case decreases by a factor of $e^{-1} = 0.368$ for people with education level 6-11 years compared to people with education level 0-5 years, controlling for all other predictors.\
The odds of being a case decreases by a factor of $e^{-1.4} = 0.247$ for people with education level 12+ years compared to people with education level 0-5 years, controlling for all other predictors.
...
lmod4 |>
  ...
  bold_p(t = 0.05)
```
**Answer:** The coefficients and p-values do not change much from the previous model. Every one unit increase in the number of prior spontaneous abortions increases the odds of being a case by a factor of $e^{2} = 7.39$, controlling for all other predictors. Every one unit increase in the number of prior induced abortions increases the odds of being a case by a factor of $e^{1.3} = 3.67$, controlling for all other predictors.
...
```{r}
lmod5 <- clogit(case ~ ordered(spontaneous) + ordered(induced) + 
...
lmod5 |>
  ...
  bold_p(t = 0.05)
```
**Answer:** When considering lienar effects, the p-values of `ordered(spontaneous)` and `ordered(induced)` are both $<0.001$. This means that the linear effects of these two predictors are significant. When considering the quadratic effects, the p-values of `ordered(spontaneous)` and `ordered(induced)` are $0.8$ and $>0.9$, respectively. This means that the quadratic effects of these two predictors are not significant. So the ordinal representation is not necessary because we can check linear effects even if we do not use ordinal factors.
