...
&=& \sum_{i} \left[y_{i} \mathbf{x}_{i} - \frac{e^{\mathbf{x}_{i}^T \boldsymbol{\beta}} \cdot \nabla_{\beta} \mathbf{x}_{i}^{T} \boldsymbol{\beta}}{1 + e^{\mathbf{x}_{i}^{T} \boldsymbol{\beta}}} \right] \\
&=& \sum_{i} \left[y_{i} \mathbf{x}_{i} - \frac{e^{\mathbf{x}_{i}^T \boldsymbol{\beta}} \cdot \mathbf{x}_{i}}{1 + e^{\mathbf{x}_{i}^{T} \boldsymbol{\beta}}} \right]
\end{eqnarray*}

So the Hessian matrix is as follows:

\begin{eqnarray*}
H_{\beta} &=& \nabla^{2}_{\beta} \ell(\boldsymbol{\beta}) &=& \sum_{i} \left[-\nabla_{\beta}\frac{e^{\mathbf{x}_{i}^T \boldsymbol{\beta}} \cdot \mathbf{x}_{i}}{1 + e^{\mathbf{x}_{i}^{T} \boldsymbol{\beta}}}\right] \\
&=& \sum_{i} \left[-\nabla_{\beta}\frac{1 \cdot \mathbf{x}_{i}}{1 + e^{-\mathbf{x}_{i}^{T} \boldsymbol{\beta}}}\right] &=& \sum_{i} \left[-\nabla_{\beta}(1 + e^{- \mathbf{x}_i^T \boldsymbol{\beta}})^{-1} \cdot \mathbf{x}_{i}\right] \\
&=& \sum_{i} \left[\frac{e^{-\mathbf{x}_{i}^T \boldsymbol{\beta}} \cdot -\nabla_{\beta} \mathbf{x}_{i}^{T} \boldsymbol{\beta}}{(1 + e^{-\mathbf{x}_{i}^{T} \boldsymbol{\beta}})^{2}} \cdot \mathbf{x}_{i} \right] \\
