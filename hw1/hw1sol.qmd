...
```{r}
quantile(pima$bmi, 0.75) - quantile(pima$bmi, 0.25)
```

```{r}
confint(pima_model3)
```
**Answer:** The $IQR$ for `bmi` is $9.3$. So the difference in the odds of testing positive for diabetes for a women with a BMI at the first quartile compared with a women at the third quartile is $e^{0.088 \times 9.3} = 2.267$. The $95\%$ confidence interval for log-odds with respect to `bmi` is $(0.059, 0.117)$. So we first times $9.3$ to get the $95\%$ confidence interval for log-odds $(0.5487, 1.0881)$, then take exponential of these values and then the $95\%$ confidence interval for odds is $(1.73, 2.97)$. This means that we are $95\%$ confident that the odds of testing positive for diabetes for a woman with a BMI at the third quartile are between $1.73$ and $2.97$ times higher than the odds of testing positive for diabetes for a woman with a BMI at the first quartile, assuming that all other factors are held constant.
