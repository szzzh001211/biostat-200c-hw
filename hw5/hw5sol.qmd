...
```{r}
ranef_smod <- ranef(smod)$operator
SSA <- sum((ranef_smod)^2)
residuals_smod <- resid(smod)
SSE <- sum(residuals_smod^2)
SSA/20-SSE/(20*4)
SSE/((5-1)*4)
```
...
$$
(\sigma^{-2} Z^T Z + \Sigma^{-1})^{-1} = \Sigma - \Sigma Z^T (\sigma^2 I_n + Z Z^T)^{-1} Z \Sigma.
$$
...
\mathbb{E} (\boldsymbol{\gamma} \mid \mathbf{y})&=& \frac{n}{n + (\sigma_{\epsilon}/\sigma_{\alpha})^2} \begin{pmatrix}
