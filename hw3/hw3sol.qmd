...
$$
Since $\mathbf{x}_{i} = \begin{pmatrix} 1 \\ \mathbf{x}_{i1} \\ \vdots \\ \mathbf{x}_{iq} \end{pmatrix}$, then if we consider the first row of $\mathbf{x}_{i}$, we can get
$$
...
```
**Answer:** The plot shows that people with chronic conditions but not limited in activity have higher `doctorco` than the other two conditions, especially when the number of `doctorco` is less than 2.5. People with chronic conditions and limited in activity do not have higher `doctorco`, probably because they are limited in activity.
...
```
**Answer:** The lines of observations are due to the fact that the `doctorco` is a discrete variable (count variable). 

```{r}
qqnorm(residuals(mod1, type = "response"))
```
**Answer:** The QQ plot of the residuals shows that the residuals are not normally distributed.
### Q3.5
...
```
**Answer:** Since there are so many zeroes, it is worth fitting a zero-inflated count model.
...
```
**Answer:** When the predicted values of the Poisson model are small, the predicted values of the Gaussian model are larger. When the predicted values of the Poisson model are large, the predicted values of the Gaussian model are smaller. The predicted values of the Gaussian model can be negative, which is not possible for the Poisson model.
